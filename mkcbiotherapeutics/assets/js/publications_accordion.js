...
export function renderAccordionItem(container, item, serialNumber) {
  ...
  accordionItem.innerHTML = `
    <div class="accordion-content" style="display: block;">
      <p class="accordion_title">${item.content}</p>
      <p class="accordion_author">Author - ${item.author}</p>
      
      ${item.journal ? `<p>Journal - ${item.journal}</p>` : ''}

      <!-- Conditionally render Impact Factor if available -->
      ${item.impactFactor ? `<p class="">Impact Factor: ${item.impactFactor}</p>` : ''}
      ${item.impactFactor2019 ? `<p class="">Impact Factor in 2019: ${item.impactFactor2019}</p>` : ''}
